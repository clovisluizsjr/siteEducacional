...
class AtividadeAlunoModel{
    ...
    constructor(atividadeAluno_id, aluno_RA, atividadeAluno_notaAluno, atividadeAluno_feedbackProf, atividadeAluno_prazoEntrega, atividadeProf_idProf, disciplina_id) {
    ...
    async listarAtivideadesPorAluno(aluno_RA){
        let sql = "SELECT * FROM AtividadeAluno WHERE aluno_RA =?";       //filtra por RA
        let valores = [aluno_RA]
        let banco = new Database();
        let lista = [];
        let rows = await banco.ExecutaComando(sql, valores);
        for(let i= 0; i < rows.length; i++) {
            lista.push(new AtividadeAlunoModel(rows[i]["atividadeAluno_id"],
                                    rows[i]["aluno_RA"],
    ...
    async ExibirNomeDisciplina(disciplina_id){
        let sql = "SELECT disciplina_nome FROM Disciplinas WHERE disciplina_id =?";
        let valores = [disciplina_id]
        let banco = new Database();
        let rows = await banco.ExecutaComando(sql, valores);
        return rows[0]["disciplina_nome"];
    }
